...
func (s *testParserSuite) TestBuiltin(c *C) {
	table := []testCase{
		...
		{`select hll_distinct_count(c1) from t;`, true, "SELECT HLL_DISTINCT_COUNT(`c1`) FROM `t`"},
		{`select hll_distinct_count(c1, c2) from t;`, true, "SELECT HLL_DISTINCT_COUNT(`c1`, `c2`) FROM `t`"},
		{`select hll_distinct_count(c1, 123) from t;`, true, "SELECT HLL_DISTINCT_COUNT(`c1`, 123) FROM `t`"},
